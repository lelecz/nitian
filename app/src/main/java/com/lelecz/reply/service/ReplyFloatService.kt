...
class ReplyFloatService : Service() {
    ...
    override fun onCreate() {
        ...
        // 启动 AlarmManager 保底检查（进程被杀也能拉起）
        AccessibilityGuard.startAlarmGuard(this)
    }

    // ── 无障碍守护：定时检查，被系统关了立刻通知引导恢复 ──
    private val guardJob = object : Runnable {
        override fun run() {
            try {
                AccessibilityGuard.notifyIfClosed(this@ReplyFloatService)
            } catch (_: Exception) {}
            mainHandler.postDelayed(this, AccessibilityGuard.getCheckIntervalMs())
        }
    }
    ...
    private fun showBubble(lastMsg: String, replies: List<String>) {
        ...
        // 防御：布局加载失败（如主题属性解析异常）时静默降级，绝不让服务崩溃
        val view = try {
        ...
        refreshBtn.setOnClickListener {
            val hasService = NotifyAccessibilityService.hasInstance()
            if (hasService) {
                Toast.makeText(this, "🔄 重新生成中…", Toast.LENGTH_SHORT).show()
                NotifyAccessibilityService.refreshReplies()
            } else {
                // 无障碍服务实例不存在：可能是被系统杀掉了还没恢复
                // 先尝试检查状态，如果服务已关闭则通知用户
                val enabled = AccessibilityGuard.isEnabled(this)
                if (enabled) {
                    // 系统显示已开启但 instance 为 null，说明服务异常
                    Toast.makeText(this, "服务正在恢复，请稍候再试…", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this, "无障碍服务未开启，无法刷新", Toast.LENGTH_SHORT).show()
                    AccessibilityGuard.notifyIfClosed(this)
                }
            }
        }

        // 拖动：只挂在标题文字手柄上，完全不拦截按钮点击
